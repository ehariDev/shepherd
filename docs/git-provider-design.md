...
> The one item that stayed open after shipping — `ssh` in the e2e compose stack (F9-a) — was fixed 2026-09-10 (go-git's `HostKeyAlgorithms` fallback, `internal/gitrepo/transport.go`) and runs green in `make e2e`.
