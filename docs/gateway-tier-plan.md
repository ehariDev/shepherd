...
- **R6 — signed, conditional.** Agent proposals must **never** reach the auto-applying `gitsync`
  ...
  **per-service-account request rate limit**, and (2) `propose_pipeline_revision` records a
  ...
  human (today the text path leaves no row). A sandbox-run budget was considered and not required.
