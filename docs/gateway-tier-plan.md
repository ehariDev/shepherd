...
> Status (re-checked 2026-09-11 against §9): **W1, W2, W3 and W8 are done. W4, W5, W6, W7, W9, W10
> and W11 are built and gated** — every G-gate in §6 is closed; the review sign-offs in §7 are
> mixed: **R2 and R3 are still unsigned, R6 is only partly resolved; R1 is ready for sign-off and
> R5 is resolved** (§7, "Decisions taken against these gates so far"). W6 and W7 are additionally
> gated on being wired into a caller — the library and its tests exist, nothing in the product
> invokes them yet. **Status and reality already diverge in one place**: the beacon ships on by
> default (D6) ahead of R2's sign-off (§7, "The beacon reaches real deployments at v0.0.2"). Treat
> every other gated workstream as not user-reachable until its gate closes. This document is the
