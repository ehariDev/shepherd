...
Mount the generated handler at `POST /collector.v1.CollectorService/{Method}` on the main HTTP server, wrapped in h2c so both HTTP/1.1+JSON and HTTP/2+proto work over cleartext. When `server.tls.*` is configured (native TLS, §14), h2c is not used — HTTP/2 is negotiated over ALPN instead, and the handler mount is otherwise identical. Read `local_attributes`, falling back to deprecated `attributes` if `local_attributes` is empty.
...
A pipeline has a list of matcher strings using **Prometheus Alertmanager matcher syntax**: `cluster="prod-eu-1"`, `role!="logs"`, `cluster=~"prod-.*"`, `env!~"dev.*"`. Parse them with `github.com/prometheus/alertmanager/pkg/labels.ParseMatcher`. All matchers on a pipeline are **ANDed**. They evaluate against the label set of the target logical collector: `{cluster: <cluster name>, role: <role>}` plus, when the owning org has opted in independently to each (`allow_label_matching` / `allow_local_attribute_matching`, procoduck/shepherd#139), the collector's admin-set "Manage labels" and its agent-reported `local_attributes` — precedence `{cluster, role} > adminLabels > localAttrs`, reserved keys (`cluster`, `role`, `id`, `os`, `alloy_version`, `collector.*`, `shepherd.*`) dropped from either source (`internal/merge.IsReserved`). **`local_attributes` is the single most-recently-seen live instance's whole attribute blob, not a true per-key union across every live instance** — a deliberate simplification (LABEL-MATCHING-PLAN.md §5, decided 2026-09-18): a collector with more than one genuinely concurrent live instance (an HA pair, or briefly during a rolling upgrade) reporting different keys can see served config flap depending on poll timing. A pipeline with zero matchers matches **nothing** (safety default — the UI must always require at least one matcher).
...
server:    { listen: ":8080", base_url: "https://shepherd.example.internal" }
           # tls: { cert_file: "", key_file: "", min_version: "1.2", client_auth: "none",
           #        client_ca_file: "", reload_interval: "30s", collector_ca_file: "" }
           # Native HTTPS: enabled iff both cert_file and key_file are set (no separate flag).
           # Cleartext h2c stays the default. Certificates hot-reload (poll + SIGHUP); a failed
           # reload keeps serving the last good certificate. See internal/config.TLSConfig.
...
| `service.yaml` / `service-metrics.yaml` | `service.yaml`: ClusterIP, port 8080 → `http`. **`appProtocol`** on the port from `service.appProtocol` (default `kubernetes.io/h2c`, so Envoy/kgateway negotiates HTTP/2 cleartext to the pod for agents using the gRPC/Connect+proto path; set to `""` for NGINX Gateway Fabric, which refuses h2c upstreams — Shepherd serves HTTP/1.1 equally well). **When `tls.enabled`**: the port's name/`targetPort` become `https` and `appProtocol` is forced to `https` regardless of `service.appProtocol` (h2c is meaningless once TLS is on); `service.port` itself is unaffected — it maps to the container's real port by name, not by number. `service-metrics.yaml`: a second, always-ClusterIP Service (toggle `metrics.enabled`) exposing only the metrics port — deliberately not a port on the main Service, since `service.type` is operator-configurable and adding metrics there would publish it however the main Service is exposed. |
| `certificate.yaml` | Toggle `tls.certManager.enabled` (default off): renders a cert-manager `Certificate` (`issuerRef`, `dnsNames`, `secretName` — defaulting to `<fullname>-tls`) for the TLS Secret the Deployment mounts. `pre-install,pre-upgrade` hook at the same weight as the CNPG Cluster/ExternalSecret (`shepherd.bootstrapHookDeps`, so it exists before the migration Job's hook needs to mount it) — unlike those two, not `lookup`-guarded against re-issuing on every `helm upgrade`, since losing a certificate is not the same class of harm as losing a database or an unrotatable key. Requires the cert-manager operator, which this chart does not install. |
...
replicas: 2
service: { ... }
tls: { enabled: false, secretName: "", port: 8443, minVersion: "1.2",
       collectorCA: { secretName: "", key: ca.crt },
       certManager: { enabled: false, issuerRef: { name: "", kind: ClusterIssuer }, dnsNames: [] } }
