...
{
  "kind": "alloy-graph/v1",
  "schema_version": "alloy-v1.18.1",        // the schema artifact this graph was authored against
  "nodes": [
    {
...
```
POST /api/orgs/{org}/visual/render     [orgeditor] graph → { content, diagnostics[], node_map }   // node_map: alloy block ↔ node id, for error mapping
...
GET  /api/orgs/{org}/simulate/runs/{id} [orgeditor] → status | results (captured series/lines, component health)
```
...
| Fan-in | An input's cardinality comes from the schema: list-typed inputs (`forward_to`, `targets` via concat) accept MULTIPLE incoming wires — codegen emits the list/`array.concat`; scalar reference props accept exactly one. **As built** (`store.ts`'s `addEdge`, W5-03): a second wire to a scalar port REPLACES the existing one immediately, as one undo step — no blocking confirm dialog; `CanvasPane`'s `onConnect` shows an Undo toast only when a wire was actually replaced, not on every connect. |
...
`remote.kubernetes.secret` and `local.file` are **config nodes**: they render on canvas (Config category tint, no stream ports) but connect via **bindings**, not wires — a secret-typed or expression-capable property in the inspector opens a picker listing on-canvas config nodes (+ "add secret node" shortcut). **As built** (`web/src/visual/bindings.ts`, W5-01/W5-02): selecting one does NOT write a `doc.bindings[]` entry — it stores `{"$expr": "<expr>"}` directly in `props`, AT THE PROP'S OWN INSTANCE PATH, top-level or nested inside a repeatable block instance at any depth (proven by the `bindings-secret` corpus fixture's `n4`, `endpoint[0].basic_auth.password`). Both renderers (`render.go`, `renderTS.ts`) already emit any props value shaped `{"$expr": ...}` verbatim at any depth, so writing it into `props` is sufficient — no separate plumbing. `GraphBinding`/`doc.bindings[]` is a separate, pre-existing channel, left deliberately untouched by the new UI: it is an **import-only** path for a flat, top-level prop binding (`internal/visual/parse.go`'s round-trip of an existing config) and cannot represent a nested path — pushing a nested path into it would emit invalid Alloy on both renderers, a known, accepted gap (see the workstream's RISKS list). The rendered dependency (e.g. `remote.kubernetes.secret.dest.data["url"]`) creates the Alloy-level edge. Destination presets (§2.2) are exactly: write component + secret config node + bindings, instantiated from the org destination record — the same rendering path the app-observability wizard templates use, now reified as graph fragments.
...
`/pipelines/:id/graph` (any source): server parses the content with the existing syntax parser, extracts component blocks + reference expressions into a best-effort graph (unmapped expressions render as a grey "expr" chip on the edge; anything unparseable into topology renders the node with an "opaque" badge). ~~Dagre-laid-out~~ — **built without dagre** (§2.0: never adopted): `internal/visual/parse.go`'s `ParseAlloy` places nodes on a fixed left-to-right grid (280px column step, wrapping to a new row past x=1120, 200px row step) in source order; `GraphViewPage.tsx` renders the positions the server sent as-is. Zero editing affordances, one button: "Recreate as visual pipeline" → scaffolds a NEW visual pipeline from the extraction for the user to complete (explicitly lossy, confirm dialog says so). This is the only text→graph path and it is a copy, never a link.
...
#!/usr/bin/env bash
set -euo pipefail
source deploy/versions.env                                   # ALLOY_VERSION=v1.18.1
SRC=$(mktemp -d)
...
2. Opening a graph whose `schema_version` < current → **upgrade banner** in the pipeline inspector: "Authored against alloy-v1.17.0; current is v1.18.1 — Review upgrade" (illustrative example; the current pin is `deploy/versions.env`'s `ALLOY_VERSION`, `v1.18.1` as of 2026-09-11).
...
- **Relabel trace** (`SimulateService.SimulateRelabel`, `internal/simulate/relabel.go`): rules from a selected `discovery.relabel`/`prometheus.relabel` node (or the whole chain of relabel nodes upstream of a selection) + sample targets → per-target, per-rule trace using `github.com/prometheus/prometheus/model/relabel` directly. Response: for each input target, each rule's effect (kept/dropped/labels before→after), final surviving set. Sample sources, in the picker: (a) **built-in fixtures only, as shipped** (`internal/simulate/stub_fixtures.go`'s curated k8s pod-discovery label sets) — `BottomDrawer.tsx`'s relabel panel calls `simulateRelabel` with `sample_targets: []` and no source picker at all, labelled "Uses built-in k8s fixture targets". ~~(b) paste JSON, (c) **live**: a real-target query gated by a per-org config flag~~ — **not built** (verified 2026-09-11: that config flag does not exist, there is no paste-JSON UI, and no code anywhere queries a destination's Prometheus for sample targets).
...
Around that: simulator pod: no service account token, NetworkPolicy egress limited to the harness itself (deny-all otherwise — a malicious graph can't exfiltrate; note this also means graphs scraping real cluster targets don't work in S3 by design), CPU/mem limits, non-root, the same distroless base. The graph arriving at the simulator has already passed RBAC — **org-editor**, not org-admin (`internal/mgmtapi/router.go`: "org-editor: SimulateService (D6 — settled at org-editor on every layer...)" — the router comment cites this document by name), the same tier as a pipeline save via `/visual/render`/`/visual/validate` (§1.2, corrected above).
...
**Corpus sync (infra, changed 2026-09-11):** the TS generator consumes THE SAME files, but not via a copy any more — `web/src/visual/renderTS.test.ts` reads `internal/visual/testdata/corpus/` **in place** (`goCorpusDir = join(__dirname, '../../../internal/visual/testdata/corpus')`), and `web/src/visual/__fixtures__/` is empty (`.gitkeep` only). `make generate-corpus` regenerates the Go goldens from the shipped schema artifact and no longer copies anything to `web/`; there is no `diff -r` drift check because there is nothing left to drift — one corpus, one location, two readers.
