...
// generatedAtLineRe matches buildHeader's "// Generated by Shepherd <version>
// at <RFC3339>" line -- always the first line of any content buildHeader
// produced -- so HashStableContent can normalize it before hashing.
var generatedAtLineRe = regexp.MustCompile(`(?m)(^// Generated by Shepherd .* at ).*$`)

// HashStableContent hashes content the same way Assemble's own Hash does:
// with the header's live generatedAt timestamp normalized to
// hashTimestampPlaceholder first, so hashing byte-identical content (modulo
// that timestamp) twice produces the same result (PR-144 review §11b).
//
// A caller that mutates Content after Assemble -- e.g. appending a beacon
// baseline pipeline, as serve.ComputeServed does -- must hash the final
// content with this instead of HashContent directly, or the timestamp is
// baked back into Hash and every recompute churns it again, defeating the
// stability Assemble's own Hash already provides.
func HashStableContent(content string) string {
	return HashContent(generatedAtLineRe.ReplaceAllString(content, "${1}"+hashTimestampPlaceholder))
}
